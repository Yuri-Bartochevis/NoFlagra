# NO FLAGRA — edge (Frigate + receiver + firmware)

This is the part that runs at the gym: Frigate recording the mat camera
continuously, the receiver that cuts a clip on a button press, and the ESP32
firmware for the physical button. It works standalone today (one gym, one
shared secret, clips on local disk) exactly as it always has — the root
[README's "Where this is going"](../README.md#where-this-is-going--no-flagra-as-a-product)
section explains how this becomes the capture/export/upload half of the
larger multi-gym product.

All commands below assume your terminal is open **in this `edge/` folder**.
...
| Path | What it is |
|---|---|
| `docker-compose.yml` | Defines the containers: Frigate and the receiver |
| `.env` | Your secret password (hidden file — see below) |
| `frigate/config.yml` | Frigate settings for the camera |
...
- A terminal opened **in this `edge/` folder**. On Mac: right-click the
  folder → Services → "New Terminal at Folder".
...
### 2. Point Frigate at your camera

Open `frigate/config.yml` and replace the `path:` line with the RTSP address
from the Mibo app. Percent-encode any special characters in the password:
`@` becomes `%40`, `#` becomes `%23`.

### 3. Start it

```
docker-compose up frigate receiver
```
...
Open <http://localhost:5000>. You should see the mat.
...
— you should see the ten minutes of mat footage leading up to your press.
...
## The NO FLAGRA dashboard (port 5001)
...
| Task | Command |
|---|---|
| Start (foreground) | `docker-compose up frigate receiver` |
| Start (background) | `docker-compose up -d frigate receiver` |
...
folder you created it in, so create it in this `edge/` folder and not a copy
of it elsewhere. And every new terminal window needs
`source .venv/bin/activate` again before Python will find the packages —
"ModuleNotFoundError" almost always means you skipped that line.
...
**1. Camera alone.** Put the real RTSP URL in `frigate/config.yml`, then
`docker-compose up -d frigate`. Confirm you see the mat at
...
services instead: `docker-compose up frigate receiver`.
...
`.env` is gitignored because it holds the shared secret. From this `edge/`
folder:

```
cp .env.example .env      # put your own secret in it
docker-compose up -d --build frigate receiver
```
